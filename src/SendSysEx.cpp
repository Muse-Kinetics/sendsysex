...
void printHelp()
{
    std::cout
        ...
        << "\nModes:\n"
        << "  1. Raw send      Send a .syx file straight to a raw MIDI output port.\n"
        << "  2. Auto update   Hand off to the family database for firmware updates.\n"
        << "\nUsage:\n"
        << "  SendSysEx -l\n"
        ...
        << "  For the legacy trojan-bootloader install and its --bl-* tools, see --help-bootloader.\n"
        << "\nPorts:\n"
        << "  -l                          List all raw MIDI input and output ports.\n"
        << "  --list-normalized [family]  Show raw port names and their normalized forms\n"
        << "                              (default family: SoftStep).\n"
        << "  -p <num>                    Select a raw output port by number.\n"
        << "  -n <name>                   Select a raw output port by exact name.\n"
        << "  --app-port <name>           Exact port name to use, bypassing family-marker\n"
        << "                              discovery (for hubs whose port name doesn't match the\n"
        << "                              attached product, e.g. a K-Board behind a Mimic Hub).\n"
        << "                              State is still read from the id reply's PID MSB.\n"
        << "  --bootloader-port <name>    Exact port name to use once in bootloader mode, if\n"
        << "                              different from --app-port. Requires --app-port.\n"
        << "\nRaw send mode:\n"
        << "  -f <file>                   .syx file to send to the selected port.\n"
        << "\nAutomatic update mode:\n"
        << "  --fw-update <family>        Update a family to firmware from the database. One of:\n"
        ...
        << "                              or reboot.\n"
        << "\nTransfer options (chunked send, listed in the order applied):\n"
        << "  -cs, --chunk-size <bytes>   Chunk size (default: 512).\n"
        << "  -fcs, --first-chunk-size <bytes>  Size of the first chunk only (default: 0 = use\n"
        << "                              -cs). Pairs with -fgd: clear the host fast, then settle.\n"
        ...
        << "\nGlobal options:\n"
        << "  --midi-backend <winmm|wms>  Windows only. Force a backend instead of auto-\n"
        << "                              detecting. 'wms' fails if the Windows MIDI Services\n"
        << "                              runtime isn't present rather than falling back to WinMM.\n"
        << "  --timestamp                 Prefix every output line with [HH:MM:SS.mmm].\n"
        << "  -h, --help                  Show this help.\n"
        << "  --help-bootloader           Show the legacy trojan-bootloader install help.\n"
        << "\nNotes:\n"
...
void printBootloaderHelp()
{
    std::cout
        ...
        << "\n"
        << "  These units shipped with NO bootloader. A captured monolithic firmware image\n"
        << "  installs one in-place: ONE F0...F7 of many KMI packets (2 banks of FW_HEADER +\n"
        << "  [FW_BLOCK_HEADER, FW_DATA] sectors), sent as a single message - the receiver\n"
        << "  resets its stage flag on every F0.\n"
        << "\n"
        << "  WARNING: the install erases the reset vector mid-flight; a failure can brick the\n"
        << "  unit (recoverable only over SiLabs USB/JTAG). Send only to a unit you can re-flash.\n"
        << "  On Windows, --bl-send REQUIRES Windows MIDI Services (--midi-backend wms): WinMM\n"
        << "  can make this device surprise-disconnect mid flash-write. WMS is hardware-validated.\n"
        << "\nUsage:\n"
        << "  SendSysEx --bl-decode -f <legacy.syx>\n"
        << "  SendSysEx --bl-send -f <legacy.syx> -p <num>|-n <name>|--family <family> [options]\n"
        << "  SendSysEx --bootloader-install <family>\n"
        << "  SendSysEx --send-bl-erase-reboot-cmd --pid <pid> -p <num>|-n <name>\n"
        << "\nCommands:\n"
        << "  --bl-decode                 Decode an image and print its bank/sector manifest\n"
        << "                              (offline, opens no port). Inspect/verify CRCs.\n"
        << "  --bl-send                   Stream an image (-f) to the port one sector at a time\n"
        ...
        << "                              and -p/-n.\n"
        << "\nSelection & output:\n"
        << "  --family <name>             With --bl-send, auto-select the family's control-\n"
        << "                              surface output port instead of -p/-n (handles legacy\n"
        << "                              SoftStep's unnamed \"SSCOM\" ports).\n"
        << "  --out-dir <path>            Directory for --bootloader-install's session log +\n"
        << "                              captured image (default:\n"
        << "                              ./bootloader-install-<family>-<timestamp>).\n"
        << "  --step                      Pause at each sector (Enter=next, N=send N, a=auto,\n"
        << "                              q=quit) to watch the device sector by sector.\n"
        << "  --dry-run                   Print the send schedule without opening a port.\n"
        << "\nProbe (pre-send connectivity check):\n"
        << "  --probe                     Send a REQUEST_FW_VERSION first and require a reply,\n"
        << "                              else abort. The legacy app has no Universal Inquiry;\n"
        << "                              the v93 reply arrives ~1.35s later.\n"
        << "  --probe-only                Probe, print the result, and stop - send no firmware.\n"
        << "  --probe-timeout <ms>        Wait for the probe reply (default: 4000; keep >2000).\n"
        << "\nTiming (in the order applied during a send):\n"
        << "  --padding-scale <0..1>      Fraction of the image's baked-in inter-packet padding\n"
        << "                              to keep (default 1.0 = wire bytes identical to the file,\n"
        << "                              the proven-safe stream; gaps add on top).\n"
        << "  --strip-padding             Alias for --padding-scale 0. Drops ~19.7 KB of padding\n"
        << "                              - misframes real hardware; experiment only.\n"
        << "  --first-block-data-gap <ms> Override --block-data-gap for the first sector only\n"
        << "                              (default: 0 = use --block-data-gap).\n"
        << "  --block-data-gap <ms>       Gap after each FW_BLOCK_HEADER, before its FW_DATA -\n"
        << "                              the sector's flash-erase time (default: 40).\n"
        ...
        << "  -pd, --post-delay <ms>      Wait after the final F7 (default: 500).\n"
        << "\nVerify (after the send):\n"
        << "  --verify                    Wait for the reboot, run the id request, and confirm\n"
        << "                              the unit came up in bootloader mode (install success).\n"
        << "                              Exit code reflects the result.\n"
        << "  --verify-settle <ms>        Wait before the first verify id request (default: 6000;\n"
        << "                              the trojan needs ~5s to install and reboot).\n"
        << "  --verify-timeout <s>        Keep retrying the id request after the settle\n"
        << "                              (default: 15).\n"
        << "\n--bank-probe (deprecated - the blind --bank-gap is proven; do not use):\n"
        << "  --bank-probe                At the bank transition, send a REQUEST_FW_VERSION packet\n"
        << "                              inside the open message and wait for the reply before\n"
        << "                              bank 1. Supersedes --bank-gap.\n"
        << "  --pre-probe-delay <ms>      Wait before the first bank probe (default: 250).\n"
        << "  --bank-probe-timeout <ms>   Total budget for a probe reply (default: 12000).\n"
        << "\nExample:\n"
        << "  SendSysEx --bl-send -f Softstep-99-trojan.syx --family SoftStep --verify\n\n";
}
...
CliOptions parseArguments(int argc, const char *argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "--help-bootloader")
        {
            options.showBootloaderHelp = true;
            return options;
        }
        if (arg == "-l")
        {
